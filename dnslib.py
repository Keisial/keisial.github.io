...
class DnsResult:
	...
	self.header['opcodestr']=dnsopcode.opcodestr(self.header['opcode'])
	self.header['status']=dnsstatus.statusstr(self.header['rcode'])
	...
	q['qtypestr']=dnstype.typestr(q['qtype'])
	q['qclassstr']=dnsclass.classstr(q['qclass'])
	return q 

    def storeRR(self,u):
	r={}
	r['name'],r['type'],r['class'],r['ttl'],r['rdlength'] = u.getRRheader()
	r['typename'] = dnstype.typestr(r['type'])
	r['classstr'] = dnsclass.classstr(r['class'])
	#print 'name=%s, type=%d(%s), class=%d(%s), ttl=%d' \
	#      % (name,
	#	 type, typename,
	#	 klass, dnsclass.classstr(class),
	#	 ttl)
